...
1. How many vessels are in the data set? What is the vessel code and name?

**Vessel Code**
```{r distinct vessel code}
...
3. What are their dimensions (length, width, gross tonnage) and carrying capacity?
...
Observations:
- One of the ships has a small carrying capacity for its size.
...
```
Observations: vessel_classification doesn't give us any information. 
...
```
Looking at the maximum number of trips made
...
longline %>%
  ...
  summarise(hauls = n()) %>%
  ggplot(aes(x = Month, y = hauls, group = vessel_code, color = vessel_code)) +
  geom_line() +
...
(start_position_map <- leaflet(data = longline) %>%
  ...
  addCircleMarkers(lng = ~startlondd, lat = ~startlatdd, clusterOptions = NULL, color = ~pal(Genypterus_blacodes), radius = 2) %>%
...
(end_position_map <- leaflet(data = longline) %>%
  addProviderTiles(providers$Esri.OceanBasemap) %>%
  addCircleMarkers(lng = ~endlondd, lat = ~endlatdd, clusterOptions = NULL, color = ~pal(Genypterus_blacodes), radius = 2) %>%
    addLegend(position = "bottomright",
              pal = pal,
              values = ~Genypterus_blacodes,
              title = "Proportion of catch that is pink cusk-eel",
...
```
2. How often to species come up in the hauls?
...
What does the fishing information look like?
