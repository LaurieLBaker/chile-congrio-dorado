...
species_key <- readxl::read_xlsx("data/Maestro especies actualizado.xlsx")
...
The data has `r nrow(palangre87_97)` observations and `r ncol(palangre87_97)` variables.
...
The data has `r nrow(palangre97_18)` observations and `r ncol(palangre97_18)` variables.
...
```{r calculate trip duration longline}
...
2. When were the vessels active?
...
palangre87_97 %>%
  ...
  mutate("COD_ESPECIE" = as.numeric(substr(species_code, start = 2, stop = 4))) %>%
  left_join(species_key, by = "COD_ESPECIE") %>%
  ...
  ungroup() %>%
  select(species_code, NOMBRE_COMUN) %>%
  unique()
...
palangre87_97 %>%
  select(CBU_ORAC, crucero, lance, Year, fecha, Total.de.peso, starts_with(match = "X")) %>%
  pivot_longer(cols = starts_with(match = "X"), names_to = "species_code", values_to = "peso") %>%   mutate("COD_ESPECIE" = as.numeric(substr(species_code, start = 2, stop = 4))) %>%   left_join(species_key, by = "COD_ESPECIE") %>%
  mutate(prop = peso/Total.de.peso) %>%
  filter(peso > 0) %>%
...
species_list_10000 <- palangre87_97 %>%
  select(CBU_ORAC, crucero, lance, Year, fecha, Total.de.peso, starts_with(match = "X")) %>%
  pivot_longer(cols = starts_with(match = "X"), names_to = "species_code", values_to = "peso") %>%
  mutate("COD_ESPECIE" = as.numeric(substr(species_code, start = 2, stop = 4))) %>%   left_join(species_key, by = "COD_ESPECIE") %>%
  mutate(prop = peso/Total.de.peso) %>%
  filter(peso > 0) %>%
  group_by(species_code) %>%
  summarise(n = n()) %>%
...
palangre87_97 %>%
  ...
  ggplot() +
  geom_area(aes(x = Year, y = species_weight, fill = NOMBRE_COMUN, group = NOMBRE_COMUN), alpha = 0.7) +
  scale_color_viridis_d() +
  labs(fill = "Species",
       y = "Species Weight (kg)")
...
palangre87_97 %>%
  ...
  ggplot(aes(x = peso, y = as.factor(NOMBRE_COMUN), fill = as.factor(NOMBRE_COMUN))) +
  geom_density_ridges(alpha = 0.75) +
  scale_fill_viridis_d() +
  labs(y = "Vessel Code",
       x = "Weight (1000 kgs)", 
...
palangre87_97 %>%
  ...
  ggplot(aes(x = prop, y = CBU_ORAC, fill = NOMBRE_COMUN)) +
...
```{r calculate trip duration 97 to 18}
